...
void CustomPlugin::_addAnalyzeEntry(const QString& title, const char* qmlFile, const char* iconFile)
{
    Q_CHECK_PTR(qmlFile);
    // 'this' instance will take ownership on the QmlComponentInfo instance
    _customAnalyzeList.append(QVariant::fromValue(
        new QmlComponentInfo(title,
                QUrl::fromUserInput(qmlFile),
                iconFile == nullptr ? QUrl() : QUrl::fromUserInput(iconFile),
                this)));
...
CustomPlugin::settingsPages()
{
    if(_customSettingsList.isEmpty()) {
        _addSettingsEntry(tr("Allgemein"),     "qrc:/qml/GeneralSettings.qml",     "qrc:/res/gear-white.svg");
        _addSettingsEntry(tr("Telemetrie"),  "qrc:/qml/LinkSettings.qml",        "qrc:/res/waves.svg");
        _addSettingsEntry(tr("Offlinekarten"),"qrc:/qml/OfflineMap.qml",          "qrc:/res/waves.svg");
        if(_showAdvancedUI) {
...
QVariantList&
CustomPlugin::analyzePages()
{
    if (_customAnalyzeList.isEmpty()) {
        _addAnalyzeEntry(tr("Bilder Georeferenzieren"),    "qrc:/qml/GeoTagPage.qml",          "qrc:/qmlimages/GeoTagIcon");        
        if(_showAdvancedUI) {
            _addAnalyzeEntry(tr("Log Download"),     "qrc:/qml/LogDownloadPage.qml",     "qrc:/qmlimages/LogDownloadIcon");
            _addAnalyzeEntry(tr("MAVLink Console"),  "qrc:/qml/MavlinkConsolePage.qml",  "qrc:/qmlimages/MavlinkConsoleIcon");
            _addAnalyzeEntry(tr("MAVLink Inspector"),"qrc:/qml/MAVLinkInspectorPage.qml","qrc:/qmlimages/MAVLinkInspector");
            _addAnalyzeEntry(tr("Vibration"),        "qrc:/qml/VibrationPage.qml",       "qrc:/qmlimages/VibrationPageIcon");
        }
    }
    return _customAnalyzeList;
}
...
void CustomPlugin::paletteOverride(QString colorName, QGCPalette::PaletteColorInfo_t& colorInfo)
{
    if (colorName == QStringLiteral("window")) {
        ...
        colorInfo[QGCPalette::Dark][QGCPalette::ColorGroupEnabled]   = QColor("#3164AD");
        colorInfo[QGCPalette::Dark][QGCPalette::ColorGroupDisabled]  = QColor("#495057");
        ...
        colorInfo[QGCPalette::Dark][QGCPalette::ColorGroupEnabled]   = QColor("#3770B5");
        colorInfo[QGCPalette::Dark][QGCPalette::ColorGroupDisabled]  = QColor("#495057");
        colorInfo[QGCPalette::Light][QGCPalette::ColorGroupEnabled]  = QColor("#aeebd0");
        ...
        colorInfo[QGCPalette::Dark][QGCPalette::ColorGroupEnabled]   = QColor("#3164AD");
        colorInfo[QGCPalette::Dark][QGCPalette::ColorGroupDisabled]  = QColor("#3770B5");
        colorInfo[QGCPalette::Light][QGCPalette::ColorGroupEnabled]  = QColor("#aeebd0");
